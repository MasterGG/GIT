...
{   
    ...
    for(i = 0; i<2; i++)
    {
        for(j = estart[i]; j< eend[i]; j++)
        {
            ...
            if(i == 0)
            {
                edge[tmp[0]][tmp[1]].total_flow =  tmp[2];
                edge[tmp[0]][tmp[1]].res_flow  = tmp[2];
                edge[tmp[0]][tmp[1]].flow = 0;
                edge[tmp[0]][tmp[1]].perfee =  tmp[3];     
                edge[tmp[0]][tmp[1]].demand = 0;     
                EdgeNums[tmp[0]]++; 
              //  EdgeNums[tmp[1]]++; 
...
/*
int getRandNum(int reverse=0){
    // 返回一个随机的网络节点编号，节点连接的边越多，返回的概率越大
    int totalEdges = 0;
    for(int i=0;i<totals[0];i++){
        totalEdges += EdgeNums[i];
    }
    int max = arrMax(EdgeNums, totals[0]);
    int* a = (int*)malloc(sizeof(int)*totalEdges);
    int nn = 1;//临界点的惩罚全职
    //int nn = int(max/4-1);//临界点的惩罚全职
    int bSize = (max+1)*totals[0] - totalEdges+nn*totals[2];
    ...
    k = 0;
    for(int i=0; i<totals[0];i++){
        for(int j=0;j<max-EdgeNums[i]+1;j++){
            b[k++] = i;
        }
    }
    for(int j=0; j<totals[2];j++){
        for(int i=0;i<nn; i++){
            b[k++] = Cons2NetNode[j];
        }
    }
    //printf("max%d\n", max);
    //print_array(EdgeNums, MAX_NODE);
    //print_array(a, totalEdges);
    //print_array(b, bSize);
    //exit(0);
    int randNum =0;
    ...
    if(reverse==1){
        // 变数少的被抽到的概率越大
        randNum = (rand()%(bSize));
        //printf("r1%d", b[randNum]);
        return b[randNum];
    }else{
        randNum = rand()%totalEdges;
        //printf("r2%d", a[randNum]);
        return a[randNum]; 
    }
}
*/
...
struct SeverNodes up_servers(const int* videos_node, const int* videos_pos, int n, double possible=0.1){
    ...
    if(totals[0]<250){
        possible = 0.50;  
        cnt=1;
    }else if(totals[0]>=250 && totals[0]<600){
        possible = 0.25;  
        cnt=1;
    }else{
        possible = 0.20; 
    ...
    while(pre[t] !=-1)
    {
        ...
        while(u !=-1)
        {
          ...
          //  edge[v][u].flow = -edge[u][v].flow;
            ...
            edge[v][u].res_flow = edge[u][v].flow;
            edge[v][u].perfee = -edge[u][v].perfee;
            //edge[v][u].res_flow = edge[v][u].total_flow - edge[v][u].flow;
             
            perfee += edge[u][v].perfee;
            v = u;
            u = pre[u];
...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num,char * filename)
{
   ...
   // lastDeploy.results = MinCostMaxFlow(totals[0]+1,totals[0], lastDeploy.n_server);
    ...
    printf("loop-0:%d,loop-1:%d,loop-2:%d\n", n_loop[0],n_loop[1],n_loop[2]);
    end = clock();
    ...
    if(n_succ>=1){
        ...
        printf("\nloop-0:%d, loop-1:%d,loop-2:%d,iters:%d, min_cost:%d, min_n_server:%d, best-iter:%d\n\n", n_loop[0], n_loop[1], n_loop[2], (n_loop[0]+n_loop[1]+n_loop[2]),bestDeploy.results[1], bestDeploy.n_server, best_iter);
    }else{
        printf("\ntotalDemand:%d\n", totalDemand);
